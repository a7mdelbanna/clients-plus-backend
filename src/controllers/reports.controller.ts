...
export class ReportsController {
  ...
  async generateDailyReport(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!validation.success) {
        res.status(400).json(errorResponse('Invalid query parameters', validation.error.issues));
        return;
      }
  ...
  async generateCustomReport(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!validation.success) {
        res.status(400).json(errorResponse('Invalid request parameters', validation.error.issues));
        return;
      }
  ...
  async exportDailyReport(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!queryValidation.success) {
        res.status(400).json(errorResponse('Invalid query parameters', queryValidation.error.issues));
        return;
      }

      const exportValidation = exportReportSchema.safeParse(req.body);
      if (!exportValidation.success) {
        res.status(400).json(errorResponse('Invalid export parameters', exportValidation.error.issues));
        return;
      }
  ...
  async exportCustomReport(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!reportValidation.success) {
        res.status(400).json(errorResponse('Invalid report parameters', reportValidation.error.issues));
        return;
      }
